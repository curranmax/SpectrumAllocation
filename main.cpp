...
int main(int argc, char const *argv[]) {
	...
	PropagationModel* pm;
	// args.propagation_model = "longley_rice";
	...
	if(args.brief_out) {
		...
		for(unsigned int i = 0; i < secure_vs.size(); ++i) {
			std::cout << secure_vs[i] << ":" << plaintext_vs[i] << ":" << ground_truth_vs[i];
			if(i < secure_vs.size() - 1) {
				std::cout << ",";
			}
		}
		std::cout << std::endl;
